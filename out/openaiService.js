 ...
 */
async function generateFakeData(javaClass, count, format, apiKey) {
    try {
        // Create field list with semantic hints
        const fieldList = javaClass.fields.map(field => {
            const semantics = (0, javaParser_1.analyzeFieldSemantics)(field);
            return {
                name: field.name,
                type: field.type,
 ...
 */
function buildPrompt(className, fields, count, format) {
